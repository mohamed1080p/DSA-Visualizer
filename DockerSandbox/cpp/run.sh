...
# Decode code
echo "$CODE" | base64 -d > main.cpp

# Compile
g++ main.cpp -O2 -o app 2> compile_error.txt

# If compilation fails → print error and exit
if [ $? -ne 0 ]; then
...
if [ -n "$BATCH_INPUTS" ]; then
    ...
    while IFS= read -r encoded_input || [ -n "$encoded_input" ]; do
        ...
        start_ms=$(date +%s%3N)
        output=$(timeout "${TIME_LIMIT_SEC}s" ./app < case_input.txt 2>stderr.txt)
        exit_code=$?
        end_ms=$(date +%s%3N)
